...
const Navbar = () => {
  ...
  useEffect(() => {
    if (pathName === `${process.env.basePath || '/'}`) {
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      if (pathName === `${process.env.basePath || '/'}` || pathName === '/location' || pathName === '/amenities') {
        // Existing behavior for home page
        if (currentScrollY > lastScrollY && currentScrollY > 500 && (pathName !== `${process.env.basePath || '/'}` && pathName !== '/location' && pathName !== '/amenities')) {
          setIsVisible(false);
        } else {
          setIsVisible(true);
        }
  ...
  return (
    <nav>
      ...
      <div style={{ zIndex: '550' }} className={`w-full z-[50] ${isSticky ? 'fixed top-0' : 'absolute top-0 left-0'}`}>
        ...
        <div className={`relative bg-no-repeat bg-cover bg-right-bottom ${megaMenu ? `bg-[#27261e] backdrop-blur px-5 sm:px-10 cmd:px-14 lg:px-6 2xl:px-8 3xl:px-4 pt-2 sm:pt-6 lg:pb-10` : ``}  w-full transition-all duration-500 ${isSticky ? `bg-[#363738] sm:bg-[#27261e] shadow-lg py-1.5 sm:py-2 lg:py-1 ${isVisible ? 'translate-y-0' : `${megaMenu ? '' : '-translate-y-full'}`} duration-500` : `${pathName == `${process.env.basePath || '/'}` ? 'py-2 lg:py-1 xl:py-4 ' : `bg-[#27261e] ${process.env.basePath == '' ? 'bg-[#363738] sm:bg-transparent-local' : 'bg-[#363738] sm:bg-transparent-production '}  py-1`}  duration-500 `} flex justify-center items-center duration-500`}>
          <div className={`duration-500  xl:py-1 lg:py-2.5 w-full 2xl:container flex justify-between lg:justify-around 3xl:justify-between ${megaMenu ? 'items-center sm:items-end xl:items-center duration-1000 px-0' : 'items-center duration-1000 px-1.5 cxs:px-5 xl:px-0'} duration-1000 mx-2`}>
            <Link href={`${process.env.basePath}`} aria-label="Back to homepage" className={`${isSticky ? 'w-[130px] h-[42px] sm:w-[160px] sm:h-[50px]  duration-1000  sm:my-0' : `${pathName == `${process.env.basePath || '/'}` ? 'w-[150px] h-[50px] sm:w-[200px] sm:h-[60px] xl:w-[206px] xl:h-[50px]' : 'w-[120px] h-[50px] sm:w-[150px] sm:h-[60px] py-3'} duration-1000`} flex relative`}>
              <Image
                src={logo}
                fill
